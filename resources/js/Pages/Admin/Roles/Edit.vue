...
<template>
    <Head>
        <title>{{ trans('admin.edit') }} '{{ role.name }}'</title>
    </Head>
    ...
    <AdminLayout>
        <div class="bg-primary text-white my-10 p-5">
            <form @submit.prevent="submit">
                ...
                <div class="mt-4">
                    <InputLabel for="name" :value="trans('admin.guard_name')" />
                    <TextInput disabled id="name" v-model="form.guard_name" placeholder="Name"
                        :class="'w-full text-white p-2 rounded bg-secondary'">
                    </TextInput>
                            ...
                            <div v-for="permission in permissions">
                                <div :key="permission.id" class="flex items-center mr-4">
                                    <input v-model="form.permissions" :value="permission.id"
                                        :checked="role.permissions.some((rolePermission: any) => rolePermission.id === permission.id)"
                                        type="checkbox"
