...
let props = defineProps(['navigation'])

const form = useForm({
    name_uk: props.navigation.name['uk'],
    name_en: props.navigation.name['en'],
    url: props.navigation.url,
    type: props.navigation.type
})

const submit = () => {
    form.put(route('admin.navigations.update', props.navigation), {
        preserveScroll: true,
        onSuccess: () => {
